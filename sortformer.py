...
class EncoderBlock(nn.Module):
    def __init__(self, config: ModelConfig) -> None:
        ...
        self.attn = Attention(config.n_embd, config.n_head, config.dropout)
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, memory: torch.Tensor, causal_mask: torch.Tensor
    ) -> torch.Tensor:
...
class SortTransformer(nn.Module):
    """Permutation-invariant encoder with an autoregressive output decoder."""
