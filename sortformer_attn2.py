...
Attention ablation 2/3: decoder self-attention only. The encoder has no
self-attention and, with cross-attention gone, its memory never reaches the
decoder.
"""
...
class DecoderBlock(nn.Module):
    def __init__(self, config: ModelConfig) -> None:
        super().__init__()
        self.self_norm = RMSNorm(config.n_embd)
        self.self_attn = Attention(config.n_embd, config.n_head, config.dropout)
        self.mlp_norm = RMSNorm(config.n_embd)
        self.mlp = MLP(config.n_embd, config.dropout)

    def forward(self, x: torch.Tensor, causal_mask: torch.Tensor) -> torch.Tensor:
        normalized = self.self_norm(x)
        x = x + self.self_attn(normalized, normalized, causal_mask)
        return x + self.mlp(self.mlp_norm(x))


class SortTransformer(nn.Module):
    """Attention-free set encoder; the decoder has causal self-attention only."""
    ...
    def forward(self, inputs: torch.Tensor, decoder_inputs: torch.Tensor) -> torch.Tensor:
        # The encoder still runs, but without cross-attention its memory is unused.
        self.encode(inputs)
        ...
        causal_mask = torch.triu(
            torch.ones(
                decoder_inputs.shape[1], decoder_inputs.shape[1],
