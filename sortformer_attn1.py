...
Attention ablation 1/3: encoder self-attention only. The decoder has neither
self-attention nor cross-attention, so the encoder memory never reaches it.
"""
...
class SortTransformer(nn.Module):
    """Set encoder with self-attention; the decoder is attention-free."""
